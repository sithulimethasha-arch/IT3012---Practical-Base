...
class VisualGridHuntGame:
    ...
    def get_percept(self) -> dict:
        ...
        return {
            ...
            # LAB 3 - Step 1.1: Expose the World Model so the agent can plan/search
            # instead of only reacting to the immediate adjacent cell.
            'agent_pos': list(self.agent_pos),   # needed so the SearchAgent knows its start state
            'facing': self.facing,               # needed so the SearchAgent knows its start orientation
...
class GridGameGUI:
    ...
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=2, walls=None, agent_type='search'):
        self.root = root
        ...
        # LAB 2/3: choose which agent architecture drives the simulation
        # 'simple' = SimpleReflexAgent (will get stuck in loops)
        # 'model'  = ModelBasedAgent (remembers visited cells, escapes loops)
        # 'search' = SearchAgent (LAB 3: plans a full path using BFS/DFS/UCS before moving)
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                # The agent senses, decides (possibly by searching a full plan), then the environment executes
...
if __name__ == "__main__":
    root = tk.Tk()
    # LAB 3: agent_type can be 'simple' (Lab 2), 'model' (Lab 2), or 'search' (Lab 3).
    # For the SearchAgent, change SearchAgent.active_algo inside agent.py to 'BFS', 'DFS', or 'UCS'
    # to compare how each search strategy plans its path to the food.
    app = GridGameGUI(root, width=12, height=12, num_food=15, num_opponents=0, agent_type='search')
    root.mainloop()
